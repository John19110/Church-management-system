...
3. Run scripts in `Church.DAL/Scripts/` (newest first if unsure).
...
dotnet ef database update `
  --project Church.DAL\Church.DAL.csproj `
  --startup-project Church.API\Church.API.csproj
```

Connection string is read from `Church.API/appsettings.Development.json` (not committed to git).

## Package Manager Console

Set **Default project**: `Church.DAL`  
Set **Startup project**: `Church.API`
